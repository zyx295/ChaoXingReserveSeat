...
# 预约后天
RESERVE_NEXT_DAY = 2  # 这里改成整数2，表示后天


def get_current_time(action):
    # 获取当前时间，action=True时按北京时间（UTC+8）计算
    if action:
...
def get_current_dayofweek(action, offset_days=0):
    # 获取星期几，支持偏移天数，action=True时按北京时间（UTC+8）
    if action:
        dt = datetime.utcnow() + timedelta(hours=8, days=offset_days)
    else:
...
def login_and_reserve(users, usernames, passwords, action, success_list=None):
    ...
    current_dayofweek = get_current_dayofweek(action, RESERVE_NEXT_DAY)
    ...
    for index, user in enumerate(users):
        ...
        if current_dayofweek not in daysofweek:
            logging.info(f"Today ({current_dayofweek}) not set to reserve for user {username}")
            continue
